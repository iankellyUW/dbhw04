...
wget -o update-severity.err -O update-severity.out "http://127.0.0.1:12138/api/v1/update-severity?severity_id=4&issue_id=$x"

if grep success update-severity.out >/dev/null ; then
	echo PASS
	exit 0
fi
